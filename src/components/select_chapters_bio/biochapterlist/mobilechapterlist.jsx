...
export default function MobileBiologyChapterList() {
  ...
  const handleQuestionChange = (id, e) => {
    ...
    setChapters((prev) =>
      prev.map((ch) =>
        ch.id === id
          ? { ...ch, numQuestions: value, rows: Array.from({ length: value }, (_, i) => ({ id: i + 1, subject: "Biology", question: "What is the role of enzymes in metabolic reactions?" })) }
          : ch
      )
    );
  };
                              ...
                              {chapter.rows.map((row, i) => (
                                <tr key={row.id} className={`hover:bg-gray-50 transition ${i === chapter.rows.length - 1 ? "border-none" : "border-b border-[#E1CFFF]"}`}>
                                  ...
                                  <td className="py-1 px-3 text-center font-semibold flex items-center justify-center space-x-1">
                                    <Image src="/bio.jpeg" alt="biology" width={14} height={14} className="w-3 h-3" />
                                    <div className="text-[10px] font-semibold">Biology</div>
                                  </td>
