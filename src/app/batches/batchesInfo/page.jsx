...
const BatchesInfoPage = () => {
  ...
  const fetchBatchInfo = async () => {
    ...
    setRefreshing(true);
    try {
      const batchId = typeof window !== "undefined" ? localStorage.getItem("batchId") : null;
    ...
    <div className="mb-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between">
        <div className="mx-auto text-center"> 
          <h1 className="text-3xl font-bold text-gray-800">Batch Information</h1>
          {batchData && (
    ...
    {batchData && (
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-white rounded-lg shadow-sm p-4 border-l-4 border-green-500">
          <h3 className="text-lg font-semibold text-gray-800">Number of Students</h3>
          <p className="text-gray-700 mt-1">{batchData.no_of_students || studentsData.length}</p>
        </div>
        
        <div className="bg-white rounded-lg shadow-sm p-4 border-l-4 border-purple-500">
          <h3 className="text-lg font-semibold text-gray-800">Number of Tests</h3>
      ...
      <div className="p-4 bg-gray-50 border-b">
        <div className="flex">
          <div className="relative flex-grow">
            <input
              type="text"
              placeholder={`Search ${activeTab === "students" ? "students" : "tests"}...`}
              value={searchQuery}
              ...
              {filteredStudents.length > 0 ? (
                filteredStudents.map((student) => (
                  <tr key={student.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{student.domicileState}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{student.batchId}</td>
                  </tr>
                ))
              ) : (
                <tr>
                  <td colSpan="5" className="px-6 py-4 text-center text-sm text-gray-500">
                    No students found {searchQuery ? "matching your search" : "for this batch"}
                  </td>
                </tr>
              )}
            </tbody>
              ...
              {filteredTests.length > 0 ? (
                filteredTests.map((test, index) => {
                  ...
                  return (
                    <tr key={index} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{test.no_of_questions}</td>
                ...
                <tr>
                  <td colSpan="7" className="px-6 py-4 text-center text-sm text-gray-500">
                    No tests found {searchQuery ? "matching your search" : "for this batch"}
                  </td>
                </tr>
              )}
            </tbody>
